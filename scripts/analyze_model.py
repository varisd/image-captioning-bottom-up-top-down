...
def main(args):
    ...
    for i in range(args.k_fold):
        ...
        tstart = datetime.now()
        indices = np.random.permutation(vector_indices.shape[0])[:MAX_SIZE]
        v_indices_subset = vector_indices[indices]
        print("MAX_SIZE = {}".format(MAX_SIZE), file=sys.stderr)
        print("Min: {}".format(np.min(v_indices_subset)), file=sys.stderr)
        print("Max: {}".format(np.max(v_indices_subset)), file=sys.stderr)
        print("Modus: {}".format(np.argmax(np.bincount(v_indices_subset))), file=sys.stderr)
        print("Mean: {}".format(np.mean(v_indices_subset)), file=sys.stderr)
        ...
        msg = "Masking - {} min".format((datetime.now() - tstart).seconds / 60)
        print(msg, file=sys.stderr)
        tstart = datetime.now()

        print("Compute orig...", file=sys.stderr)
        ...
        msg = "Compute orig - {} min".format((datetime.now() - tstart).seconds / 60),
        print(msg, file=sys.stderr)
        tstart = datetime.now()

        print("Compute proj...", file=sys.stderr)
        ...
        msg = "Compute proj - {} min".format((datetime.now() - tstart).seconds / 60)
        print(msg, file=sys.stderr)
        tstart = datetime.now()

        # Correlation (only between non-equal classes)
        print("Compute corr...", file=sys.stderr)
        ...
        msg = "Compute corr - {} min".format((datetime.now() - tstart).seconds / 60)
        print(msg, file=sys.stderr)
